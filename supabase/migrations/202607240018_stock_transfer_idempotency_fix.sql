-- Use a distinct idempotency key for the inbound leg of an atomic stock transfer.
...
begin
  ...
  if destination_location_id is not null then
    ...
    insert into public.stock_movements(school_id,inventory_item_id,inventory_location_id,movement_type,quantity,unit_cost,reference_type,reference_id,performed_by,notes,idempotency_key)
    values(target_school_id,target_item_id,destination_location_id,'transfer_in',quantity,coalesce(unit_cost,v_source.average_cost),reference_type,reference_id,auth.uid(),notes,case when idempotency_key is null then null else idempotency_key||':in' end)
